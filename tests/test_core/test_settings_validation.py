...
def test_validate_settings_passes_in_debug():
    ...
    with patch.object(settings, 'DEBUG', True), patch.object(settings, 'DB_ENGINE', 'mysql'):
...
def test_validate_settings_returns_empty_for_sqlite_debug():
    ...
    with patch.object(settings, 'DEBUG', True), patch.object(settings, 'DB_ENGINE', 'sqlite'):
        warnings = settings.validate_settings()
